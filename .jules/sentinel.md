...
## 2026-05-24 - DoS Vulnerability in API Key Authentication
**Vulnerability:** The API key validation used `bcrypt` (`pwd_context.hash` and `.verify`) for every request, and did so inside an O(N) loop iterating over all keys when keys did not contain embedded IDs. This allowed a malicious user to easily exhaust CPU resources by sending invalid keys, triggering N expensive bcrypt operations per request. Furthermore, `pwd_context.hash` generated a new random salt every time, breaking exact matches for O(1) lookups.
**Learning:** Bcrypt is designed to be slow to prevent brute-force attacks on passwords. It should not be used for high-frequency API key validation or generated dynamically for comparisons. High-frequency keys should use fast hashes (like SHA-256 with a pepper) and embed the database ID in the token string (e.g., `vwk_{id}_{secret}`) to allow O(1) lookups and completely avoid O(N) scans.
**Prevention:** Always verify that token validation uses fast algorithms (SHA-256 HMAC or peppered SHA-256) and avoid O(N) loops. Embed lookup identifiers directly in generated tokens. Only use `bcrypt` for user password hashing during login, not for per-request API key validation.
